...
def parse_args():
    ...
    parser.add_argument("-i", "--input-video", nargs='+', help="Path to input H264 elementry stream (required)", required=True)
...
def tiler_src_pad_buffer_probe(pad,info,u_data):
    ...
    while l_frame is not None:
        ...
        # getfps_streams["stream{0}".format(frame_meta.pad_index)].update_fps()
        # # getfps_streams["stream{0}".format(frame_meta.pad_index)].print_data()
        # fps = getfps_streams["stream{0}".format(frame_meta.pad_index)].get_fps()
