...
class video():
    ...
    def overlay_mask(self, mask, img):
        # Overlay mask on image
        if img.shape[:2] != mask.shape[:2]:
            img = cv2.resize(img, (mask.shape[1], mask.shape[0]))
        new_mask = cv2.addWeighted(img,0.5,mask,0.5,0)
        return new_mask
    ...
    def maskshow(self, mask, img=None, colorize=False, overlay=False):
        ...
        if overlay and img is not None:
            mask_color = self.overlay_mask(mask_color, img)
        cv2.putText(mask_color, f"{self.name} shape: {mask_color.shape}", (10, mask_color.shape[0] - 10), cv2.FONT_HERSHEY_SIMPLEX, 1, (179,147,97), 1)
